...
1. **是否需要 references/ 目录？** 按技能场景是否涉及以下内容自动判断，无需询问：
   ...
   - 模板（合同、报告、表单等）
   - 工作流详细模板
2. **是否需要 scripts/ 目录？**（仅在需要执行命令行工具或数据处理时，按技能场景自动判断）
...
```markdown
---
name: {小写字母+连字符的技术标识符，全局唯一}
display_name: {中文展示名}
display_name_en: {English Display Name}
description: {一句话技能描述，含触发词，AI 用来判断何时激活}
...
{一段话描述：本 Skill 是什么、解决什么问题、产出物是什么}
...
（按需 3-6 个阶段，每个阶段明确输入和输出）

## 参考资料（如有）

- `references/{file1}.md` — {用途}
- `references/{file2}.md` — {用途}
...
> **正文必须 ≥ 200 字**（PDF §4.1）。本模板按层级填充后通常 800-1500 字，已远超下限。
