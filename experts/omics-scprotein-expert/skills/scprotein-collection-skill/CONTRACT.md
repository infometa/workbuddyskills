# CLI / SKILL 边界契约（scPROTEIN Collection 单合集收窄版）
...
| 4   | `omics list public-apps`          | —                                      | 读           | ✅ **必须带 `--parent-app 5c63718b-d24f-4a9f-9838-4177185f414a`**；其它形态禁用                     |
...
| 反例                                                                                          | 违反原则                                                            |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| SKILL 调 `omics run --wdl <path>`（form A）                                                   | 越界——本 SKILL 只能跑 scPROTEIN Collection 下的子应用                 |
| SKILL 调 `omics run --app <ApplicationId>`（form C）                                          | 越界——同上；引导用户去 `omics-task-skill`                            |
| SKILL 调 `omics run --public-app 5c63718b-d24f-4a9f-9838-4177185f414a ...`（合集本身）        | 合集（APP_COLLECTION）不可直接 run，必须先展开挑子应用               |
...
| SKILL 调 `omics list public-apps --parent-app <非本合集 AppId>`                               | 越界——本 SKILL 只服务 scPROTEIN Collection                           |
...
| SKILL 自己挑子应用（如默认替选 Stage1 或 Stage2）/ 自己挑 NF 版本 不让用户决策                | 替用户做选型决策                                                    |
| SKILL 自动按 Stage1 → Stage2 串联跑                                                           | 多 stage 顺序执行属用户决策；SKILL 一次只 run 一个子应用             |
...
```
用户："跑 scPROTEIN" / "做单细胞蛋白组分析" / "scPROTEIN 里有啥子流程"
        │
        ▼
...
解析 Apps[]: 转述子应用清单（AppId / AppName / AppDesc / NextflowVersion[]）
        │
        ▼
用户挑定具体子应用（Stage1 / Stage2 / ...）→ 记录 <selectedAppId> / <selectedAppName> / <selectedNfVersion>
```
...
| 检查项                                                                                        | 通过标准                                                                                                                                                       |
| --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SKILL 在 build_list_public_apps 时是否硬编码 `--parent-app`                                   | 全文搜 `5c63718b-d24f-4a9f-9838-4177185f414a`；只能作为 `--parent-app` 入参，不应被用户参数覆盖                                                                  |
| `omics run --public-app` 入参是否仅来自合集展开                                               | SKILL 内部记录 `<selectedAppId>` 必须严格来自上一步 `list public-apps --parent-app ...` 的 `Apps[].AppId`                                                       |
| SKILL 是否会把合集 AppId 直接拼到 run 命令                                                    | `grep -n "build_run" SKILL.md` 检查不应出现 `public_app="5c63718b-..."` 形式                                                                                    |
...
| SKILL 是否出现 `--wdl` / `--app` 这类越界 flag                                                | `grep -nE "(\\\\-\\\\-wdl\\b\|\\\\-\\\\-app\\b)" skills/scprotein-collection-skill/SKILL.md` 仅出现在"严令禁止"和"❌"反例段落                                    |
| run 前置确认是否在 SKILL.md / wrapper 中明文要求                                              | SKILL.md "能力边界"章节 + §5.2 模板存在                                                                                                                          |
| Stage1 → Stage2 是否会被 SKILL 自动串联                                                       | SKILL.md 应明文要求"分两次发起，每次只 run 一个子应用"，禁止自动 chain                                                                                            |
...
- **新增 scPROTEIN Collection 子应用**（平台在合集下加新流程）：自动通过 §3 展开发现，无需改 SKILL；如需在
...
| 版本 | 日期       | 变更                                                                                                                |
| ---- | ---------- | ------------------------------------------------------------------------------------------------------------------- |
| v1.0 | 2026-06-03 | 从 omics-task-skill v4 派生；锁定合集 AppId=`5c63718b-...`；仅允许 `list public-apps --parent-app` + form B + status + debug；明禁 Stage 自动串联 |
