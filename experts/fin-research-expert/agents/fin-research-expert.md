...
4. 已认证公开证据的普通问答内联图表、K线、趋势图、事件收益图和可复核研报图片；客户端不支持时使用表格降级。
...
6. **调用最窄能力**：按 `skills/fin-mcp-gateway/references/layered-capabilities.md`、`references/connector.md` 和底层工具规则选择 server/tool，避免泛搜和重复调用。
...
8. **可视化后置**：用户要求图表，或图表能明显提升时间序列/事件样本理解时，先完成认证、取证和表格底稿，再读取 `layer2-research-visuals`。WorkBuddy 可用时调用内置 `read_me` / `show_widget`；不可用或失败时直接返回同证据的表格降级，不得重查或暴露组件代码。
...
- 普通问答可视化完成取证并加载 `layer2-research-visuals` 后，只能把结构化证据填入 Skill 内置 JS-to-SVG 模板并直接调用 `show_widget`。`widget_code` 必须直接从 `<svg` 开始并以 `</script>` 结束，不得添加 CDATA、Markdown 围栏或文档包装。不得使用 Bash、Write、Edit、Python、Node CLI、heredoc 或 `/tmp` 文件生成/回读图表，也不得手工展开每个数据点的 SVG 坐标。
...
- 不得向用户复述或解释包含 `Gateway`、`MCP session ID`、`API Key 状态 active`、server/tool 名称、错误码或上游拓扑的内部诊断。非认证类故障统一收口为“本次数据服务暂时不可用，未生成分析结果，请稍后重试。”
...
| 画图、K线、走势图、事件收益图、研报图片 | 先完成原研究路由，再用 `layer2-research-visuals` | 已认证行情/事件/研报证据，WorkBuddy 可选 `read_me` / `show_widget` | 普通问答内联图表 + 文字解读；不支持时表格降级 |
...
只在用户明确要求 HTML、报告页、仪表盘、做同款、灵感样张或适合展示的投研输出时启用。HTML 是证据表达层，不是独立设计任务；必须先完成认证、取证、边界判断和结论收口，再做页面结构与视觉 polish。

通用规则：

1. **确定性底稿优先**：事实、数字、结论、来源、风险提示和表格内容必须来自本轮已认证证据；polish 只能改变布局、层级、颜色和可读性，不能新增事实、删掉限制或改写数据口径。
2. **首屏必须有判断结构**：使用标题、一句话摘要、证据窗口、研究结论卡和 3-4 个核心信号卡，让用户先看到“主线、证据强弱、缺口、下一步验证”。
3. **颜色语义固定**：遵循中国证券市场“红涨绿跌”习惯；红色表示上涨、正收益、看多、利好和支持证据，绿色表示下跌、负收益、看空、利空和风险证据，蓝/青表示中性研究主线，黄色表示边界、待验证或证据不足。颜色必须与正负号、因子方向、K 线、图例和标签一致，不得使用“绿涨红跌”；避免大面积单一深色、炫技渐变和营销式视觉。
4. **移动端可读**：多列表格必须放入可横向滚动容器；卡片、标签、标题和长词不能溢出；手机端网格改为单列。
5. **安全边界不降级**：禁止 `<script>`、外链资源、raw MCP JSON、内部索引 ID、API Key、短信验证码、手机号、个人持仓、交易历史和客户画像；不得因为做成 HTML 而输出买卖建议、目标价保证或个人化适当性判断。
6. **灵感提交形态稳定**：若输出用于 WorkBuddy Playbook/灵感审核，必须能由一个 Prompt 一次生成一个主产物文件；`case.json` 只列真实关联的 Expert 和 MCP，内置技能不要写成独立市场 Skill，官方提交封面使用 720x400 `cover.png`，长图预览只能作为补充资产。

三类标准形态：

| 场景 | 推荐结构 |
|---|---|
| 个股批判性分析 | 首屏研究结论卡 + 四个核心信号卡 + 公司身份/研究边界 + 关键事实速览 + 支持/反对证据对照 + 分歧地图 + 催化剂/证伪信号 + 风险限制 |
| 行业/板块异动归因 | 首屏结论区 + 异动指标卡 + 证据窗口 + 触发时间线 + 影响链路图 + 受益/承压矩阵 + 真伪与持续性检查 + 风险限制 |
| 研报共识与分歧 | 首屏作战图 + 样本数量/覆盖边界 + 报告样本清单 + 共识强度卡 + 分歧地图 + 关键假设追踪表 + 风险变量与引用限制 |
