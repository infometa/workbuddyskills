...
description:
  "腾讯健康组学平台(Omics) - CD-GPT Collection 公共应用合集专用运行助手。CD-GPT 是生成式生物基础大模型，对 DNA/RNA/蛋白质序列预训练，捕捉生物系统全分子互作关系，支持单分子分析与多分子联合分析中的预测与生成任务。仅服务于本合集子应用的导入与运行；其他公共应用、本地 WDL、项目内已有应用请改用 omics-task-skill。触发词：CD-GPT, cd-gpt, CD GPT, cdGPT, cdgpt, CDGPT, CD_GPT, CD-GPT Collection, cdgpt collection, CD-GPT 合集, Central Dogma Generative Pretrained Transformer, 中心法则, 中心法则生成式模型, CD-GPT_translation_generation, CD-GPT translation, translation generation, CD-GPT_reverse_translation_generation, CD-GPT reverse translation, reverse translation, 翻译生成, 反向翻译, DNA 翻译, DNA 转蛋白质, 蛋白质转 mRNA, mRNA 反向翻译, 序列翻译, 生物序列翻译, 生成式生物模型, 生物基础大模型, DNA RNA 蛋白质预训练, 全分子层级预训练, 单分子分析, 多分子联合分析, 单细胞与多组学基础模型, 跑 CD-GPT, 跑 cdGPT, 跑 cdgpt, 跑翻译生成, 跑反向翻译, 用 CD-GPT 翻译 DNA, 用 cdGPT 生成蛋白质, 用 CD-GPT 反向翻译, 把蛋白质反向翻译成 mRNA"
---

# CD-GPT Collection Skill (v1.0 · 单合集收窄版)

> 本 SKILL 是 `omics-task-skill` 的**单合集收窄版**：仅服务于 `CD-GPT Collection (Nextflow)` 这一个公共应用合集。
...
1. **严禁运行除 CD-GPT Collection 之外的任何应用**：
   ...
   - **唯一允许**的 run 形态：`omics run --public-app <CD-GPT 合集展开后的子应用 AppId> ...`
...
| 「跑 CD-GPT 的某子应用，参数我自己改过」  | `omics run --public-app <子应用AppId> --public-app-name <name> --nf-version <ver> --input <p>` | §5       |
...
> ⚠️ `--parent-app` 值**固定**为 CD-GPT Collection 的 AppId，不接受替换。
...
>
> - `<workflow>.input_sequence`：File（必填）
> - `<workflow>.task_type`：String（translation / reverse_translation 等，必填）
>
...
> ⚠️ `omics status` 列出的是**当前项目**所有批次（不限于 CD-GPT），SKILL 转述时
> 应**优先突出 CD-GPT 相关批次**，但不强制过滤——这是平台原生能力。
...
3. **§3 展开合集**：`list public-apps --parent-app 5e3b30ee-... -o json` → 把子应用清单转述给用户
4. 用户挑定子应用（例如 `cdGPT Translation generation`）→ 记录 `<selectedAppId>` / `<selectedAppName>` / `<selectedNfVersion>`
