...
name: scbert-skill
description:
  "腾讯健康组学平台(Omics) - scBERT 公共应用专用运行助手（独立应用，AppType=NEXTFLOW，v24.04.3）。scBERT 是基于 BERT 范式的单细胞预训练模型，高效提取临床单细胞转录组特征，可用于细胞类型注释、新类发现、marker 基因检测等肿瘤微环境分析场景，解决传统方法在标记基因依赖、批次效应处理及基因交互信息利用方面的局限性；含 FINETUNE（领域适配微调）与 PREDICT（用微调后模型预测未知细胞类型）两个核心任务。仅服务于 scBERT 这一个公共应用的导入与运行；其他应用请改用 omics-task-skill。触发词：scBERT, scbert, SCBERT, sc-BERT, sc BERT, scBert, sc_bert, 单细胞 BERT, 单细胞预训练, 单细胞预训练模型, 单细胞基础模型, 单细胞 RNA 测序, 单细胞转录组, scRNA-seq, scRNA, scRNAseq, single cell RNA-seq, 单细胞 RNA, 细胞类型注释, 细胞类型预测, 细胞类型标注, cell type annotation, cell type prediction, 新类发现, 新细胞类型发现, novel cell type discovery, marker 基因, marker 基因检测, marker gene detection, 标志基因, 标记基因, 肿瘤微环境分析, 肿瘤微环境, 单细胞肿瘤微环境, TME 分析, tumor microenvironment, FINETUNE, finetune, fine-tune, 微调, 领域适配微调, scBERT 微调, PREDICT, predict, 预测未知细胞, scBERT 预测, 批次效应处理, 基因交互信息, 跑 scBERT, 跑 scbert, 用 scBERT 注释细胞, 用 scBERT 微调, 跑单细胞类型注释, 注释单细胞类型"
---

# scBERT Skill (v1.0 · 单应用收窄版)

> 本 SKILL 是 `omics-task-skill` 的**单应用收窄版**：仅服务于 `scBERT` 这一个公共应用。
> 所有命令拼接走 `scripts/omics_cli.py`，统一参数与输出格式。
> **能力范围严格 = `omics-platform-cli` 7 命令 ∩ 仅运行 scBERT 这一个 AppId**；任何越界都视为越权。
...
| **PublicAppId**     | `publicapp-nf-scbert`                  |
| **应用类型**        | `NEXTFLOW`                             |
| **分组类型**        | `STANDALONE_APP`（独立应用）           |
| **Nextflow 版本**   | `v24.04.3`                             |
| **标签**            | AI 模型                                |

**应用简介**：scBERT 是基于 BERT 范式的单细胞预训练模型，高效提取临床单细胞转录组的特征，
可用于细胞类型注释、新类发现、marker 基因检测等肿瘤微环境分析场景。流程包含两个核心任务：

1. **FINETUNE** — 对预训练模型进行领域适配微调
2. **PREDICT** — 调用微调后的模型对未知细胞数据进行预测
...
| 「跑 scBERT」「做单细胞类型注释」       | `omics run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5 --public-app-name <name> --nf-version v24.04.3 -o json` | §4       |
| 「跑 scBERT，参数我自己改过」           | 上面命令 + `--input <path>` 显式覆盖自动模板                                                          | §4       |
...
| 「列下平台公共应用」「跑别的应用」      | ❌ **拒绝**——本 SKILL 仅服务 scBERT；建议用户改用 `omics-task-skill`                                   | —        |

> ❌ 用户说「跑本地 WDL」「跑别的公共应用 X」「跑项目里那个 app-xxx」：本 SKILL **必须拒绝**，
> 引导用户：「我只能帮你跑 scBERT；如需运行其他应用，请使用 `omics-task-skill`。」

---

## Step 3：导入前同名检查（**必经**）

scBERT 进入 `omics run --public-app ...` 之前，SKILL 必须先在 config 项目里检查是否已存在同名应用，避免：

- service 端 `CreateApplication` 因 Name 唯一约束直接报错；
- SKILL 自动加后缀（如 `scBERT-1`、`scBERT-cli`）替用户决策，污染应用列表。

### 检查方法（**例外允许的 list apps 调用**）

> 本 SKILL 整体禁用 `omics list apps`，但**仅允许在导入前同名检查这一处**调用，
> 其结果**只能用于"是否已有 scBERT 同名应用"的二选一判断**，**禁止用于让用户挑别的应用运行**。

```bash
python3 scripts/omics_cli.py list apps -o json
```

在返回 JSON 的 `Applications[]` 中查找 `Name == "scBERT"`（或用户提供的自定义 candidateName）：
...
> 我不会自动改名，请你二选一：
>
> **A. 重命名后导入（推荐）**：给一个新的导入名，比如 `scBERT-v2` / `scBERT-2026q2`
> → 我会用你给的新名字重新导入这个公共应用并运行。
>
> **B. 终止本次运行**：本 SKILL 仅服务于公共应用 scBERT 的导入与运行；
...
> - `--public-app` 值**固定**为 scBERT 的 AppId，不接受替换。
> - `--nf-version` 必须是 `v24.04.3`（scBERT 唯一可用版本；NEXTFLOW 应用必传）。
> - `--public-app-name` 按 §3 同名检查决策。
...
5. **一步直达运行**：
   ```bash
   python3 scripts/omics_cli.py run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5 \
     [--public-app-name <name>] --nf-version v24.04.3 [--name run-1] -o json
   ```
6. **若 CLI 报 `PARAM_MERGE_FAILED`**：按 §4.4 处理。
7. **如果用户主动要求改参数** → 显式传 `--input` 覆盖自动模板：
   ```bash
   python3 scripts/omics_cli.py run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5 \
     --public-app-name <name> --nf-version v24.04.3 --input ./run.json -o json
   ```
...
> 跑这次任务时 CLI 已经把 WDL/NF 的默认值和模板拼好，但还有 N 个必填项缺值：
>
> - `<workflow>.h5ad_input`：File（必填）
> - `<workflow>.task_mode`：String（FINETUNE / PREDICT，必填）
>
...
| `MISSING_NF_VERSION`                      | 本 SKILL 已硬编码 `--nf-version v24.04.3`；若 CLI 仍报错，复述给用户并提示这是 CLI 异常   |
...
紧凑总结：`最近 N 个 scBERT 批次：✅ 已完成 X / ❌ 已失败 Y / 🔄 运行中 Z`。

> ⚠️ `omics status` 列出的是**当前项目**所有批次（不限于 scBERT），SKILL 转述时
> 应**优先突出 scBERT 相关批次**，但不强制过滤——这是平台原生能力。
...
4. **修复后统一走 `omics run` 重发** —— 本 SKILL 的重发**仍然只能走 form B**：
   ```bash
   omics run --public-app 0de1b4ae-1543-4882-8bae-b5ee87968bc5 \
             --public-app-name <newName> --nf-version v24.04.3 --input <fixed.json> -o json
   ```
...
2. 用户："跑 scBERT" 或 "做单细胞类型注释"
3. **同名检查（§3）**：执行 `list apps -o json`，匹配 `Name == "scBERT"`
   - 0 条命中 → 通过
...
1. 走完场景 B 第 1~3 步
2. 用户："参数我想改 task_mode 为 PREDICT、换数据集"
...
1. **拒绝**："本 SKILL 只能帮你运行 scBERT。如需运行其他应用（本地 WDL / 其他公共应用 / 项目内已有应用），
...
scBERT 支持 GPU 资源（T4 及以上），可通过用户的 `--input run.json` 内 nextflow config 段配置：
