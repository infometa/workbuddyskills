# CLI / SKILL 边界契约（tFold Collection 单合集收窄版）
...
| 4   | `omics list public-apps`          | —                                      | 读           | ✅ **必须带 `--parent-app 807fc9ae-6197-43c0-a7a8-993c09ec1ee2`**；其它形态禁用                     |
...
| 反例                                                                                          | 违反原则                                                            |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| SKILL 调 `omics run --wdl <path>`（form A）                                                   | 越界——本 SKILL 只能跑 tFold Collection 下的子应用                     |
| SKILL 调 `omics run --app <ApplicationId>`（form C）                                          | 越界——同上；引导用户去 `omics-task-skill`                            |
| SKILL 调 `omics run --public-app 807fc9ae-6197-43c0-a7a8-993c09ec1ee2 ...`（合集本身）        | 合集（APP_COLLECTION）不可直接 run，必须先展开挑子应用               |
...
| SKILL 调 `omics list public-apps --parent-app <非本合集 AppId>`                               | 越界——本 SKILL 只服务 tFold Collection                               |
...
| SKILL 把 tFold-Ag 系列三个子流程当作同一个，自行替选其中一个                                  | 替用户做选型决策（complex / gen MSA / inter-chain feature 含义不同） |
| SKILL 自动按 tFold-Ag gen MSA → tFold-Ag complex 顺序串联跑                                   | 多 stage 顺序执行属用户决策；SKILL 一次只 run 一个子应用             |
...
```
用户："跑 tFold" / "做抗体结构预测" / "tFold 里有啥子流程"
        │
        ▼
...
解析 Apps[]: 转述子应用清单（AppId / AppName / AppDesc / NextflowVersion[]）
        │
        ▼
用户挑定具体子应用（tFold-Ab / tFold-Ag complex / tFold-Ag gen MSA / tFold-Ag inter-chain / tFold-TCR）
        │
        ▼
记录 <selectedAppId> / <selectedAppName> / <selectedNfVersion>
```
...
5. tFold-Ag 系列三个子流程必须由用户明确指定，禁止 SKILL 替选
...
| 检查项                                                                                        | 通过标准                                                                                                                                                       |
| --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SKILL 在 build_list_public_apps 时是否硬编码 `--parent-app`                                   | 全文搜 `807fc9ae-6197-43c0-a7a8-993c09ec1ee2`；只能作为 `--parent-app` 入参，不应被用户参数覆盖                                                                  |
| `omics run --public-app` 入参是否仅来自合集展开                                               | SKILL 内部记录 `<selectedAppId>` 必须严格来自上一步 `list public-apps --parent-app ...` 的 `Apps[].AppId`                                                       |
| SKILL 是否会把合集 AppId 直接拼到 run 命令                                                    | `grep -n "build_run" SKILL.md` 检查不应出现 `public_app="807fc9ae-..."` 形式                                                                                    |
...
| SKILL 是否出现 `--wdl` / `--app` 这类越界 flag                                                | `grep -nE "(\\\\-\\\\-wdl\\b\|\\\\-\\\\-app\\b)" skills/tfold-collection-skill/SKILL.md` 仅出现在"严令禁止"和"❌"反例段落                                        |
| run 前置确认是否在 SKILL.md / wrapper 中明文要求                                              | SKILL.md "能力边界"章节 + §5.2 模板存在                                                                                                                          |
| tFold-Ag 系列三个子流程是否会被 SKILL 替选                                                    | SKILL.md 应明文要求"用户笼统说 tFold-Ag 时必须再问明确是哪一个"                                                                                                  |
| Stage 自动串联（gen MSA → complex）是否会被 SKILL 自动 chain                                  | SKILL.md 应明文要求"分两次发起，每次只 run 一个子应用"，禁止自动 chain                                                                                            |
...
- **新增 tFold Collection 子应用**（平台在合集下加新流程）：自动通过 §3 展开发现，无需改 SKILL；如需在
...
| 版本 | 日期       | 变更                                                                                                                            |
| ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------- |
| v1.0 | 2026-06-03 | 从 omics-task-skill v4 派生；锁定合集 AppId=`807fc9ae-...`；仅允许 `list public-apps --parent-app` + form B + status + debug；明禁 tFold-Ag 系列替选与 stage 自动串联 |
