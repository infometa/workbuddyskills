...
description:
  "腾讯健康组学平台(Omics) - tFold Collection 公共应用合集专用运行助手。tFold 是腾讯自研的抗体/纳米抗体与抗原复合体结构预测深度学习模型系列；本合集（Nextflow，v24.04.3）含 4 个子应用：tFold_ab_predict（抗体/纳米抗体单体结构预测）、tFold_ag_predict（抗体-抗原复合物结构预测）、tFold_ag_predict_icf（加入链间特征 inter-chain feature 的抗体-抗原复合物预测）、tFold_tcr_predict（TCR 及其复合物三维结构预测）。仅服务于本合集子应用的导入与运行；其他应用请改用 omics-task-skill。触发词：tFold, tfold, TFold, t-Fold, t Fold, t_fold, tFold Collection, tfold collection, tFold 合集, tFold_ab_predict, tFold-Ab, tfold-ab, tFold Ab, ab predict, 抗体单体结构预测, 抗体结构预测, 抗体折叠, 单链抗体结构, 纳米抗体结构, 纳米抗体, nanobody, nanobody 结构预测, tFold_ag_predict, tFold-Ag, tfold-ag, tFold Ag, ag predict, 抗体抗原复合物, 抗原抗体复合物, antibody-antigen complex, 抗原复合体, 复合物结构预测, tFold_ag_predict_icf, tFold-Ag-ICF, inter-chain feature, 链间特征, ICF 模式, 链间特征复合物预测, tFold_tcr_predict, tFold-TCR, tfold-tcr, tFold TCR, tcr predict, TCR 结构预测, TCR 复合物, T 细胞受体结构, TCR-pMHC, pMHC, CDR-H3, CDR-H3 建模, DockQ, 免疫组学, 结构生物学 AI, 跑 tFold, 跑 tfold, 跑 ab predict, 跑 ag predict, 跑 ag predict icf, 跑 tcr predict, 跑抗体结构预测, 跑抗原抗体复合物预测, 跑 TCR 结构预测, 用 tFold 预测结构, 用 tFold-Ab 折叠抗体, 用 tFold-Ag 跑复合物"
---

# tFold Collection Skill (v1.0 · 单合集收窄版)

> 本 SKILL 是 `omics-task-skill` 的**单合集收窄版**：仅服务于 `tFold Collection (Nextflow)` 这一个公共应用合集。
> 所有命令拼接走 `scripts/omics_cli.py`，统一参数与输出格式。
> **能力范围严格 = `omics-platform-cli` 7 命令 ∩ 仅运行 tFold Collection 合集下的子应用**；任何越界都视为越权。
...
| **PublicAppId**          | `publicapp-nf-tfold-collection`        |
| **应用类型**             | `NEXTFLOW`                             |
| **分组类型**             | `APP_COLLECTION`（合集，含多个子应用） |
| **标签**                 | AI 模型                                |

**应用简介**：tFold 系列是腾讯自研的抗体/纳米抗体与抗原复合体结构预测深度学习模型。本合集核心子流程：
...
1. **严禁运行除 tFold Collection 之外的任何应用**：
   ...
   - **唯一允许**的 run 形态：`omics run --public-app <tFold 合集展开后的子应用 AppId> ...`
...
7. **`omics login` / `omics config set` 由用户在本机终端执行**，SKILL 永远不主动调（OAuth 浏览器
   回调 + 交互式输入只能在用户本机完成）。
8. **严禁自动按 tFold-Ag gen MSA → tFold-Ag complex 顺序串联跑**——多 stage 顺序执行属用户决策；
   一次 run 只跑一个子应用，要分两次发起。
...
| 「跑 tFold」「做抗体结构预测」「做抗原抗体复合物预测」「跑 TCR 结构预测」 | 先 §3 展开子应用清单 → 用户挑子应用 → §4 同名检查 → §5 run                                       | §3~5     |
| 「tFold 里有哪些子流程」「tFold-Ab / tFold-Ag / tFold-TCR 都有啥」 | `omics list public-apps --parent-app 807fc9ae-6197-43c0-a7a8-993c09ec1ee2 -o json`               | §3       |
| 「跑 tFold 的某子应用，参数我自己改过」                      | `omics run --public-app <子应用AppId> --public-app-name <name> --nf-version <ver> --input <p>`   | §5       |
...
| 「列下平台公共应用」「跑别的应用」                           | ❌ **拒绝**——本 SKILL 仅服务 tFold Collection；建议用户改用 `omics-task-skill`                    | —        |

> ❌ 用户说「跑本地 WDL」「跑别的公共应用 X」「跑项目里那个 app-xxx」「跑 tFold 合集本身不挑子应用」：
> 本 SKILL **必须拒绝**，引导用户：
> 「我只能帮你跑 tFold Collection 下的某一个具体子应用；如需运行其他应用，请使用 `omics-task-skill`。」
...
> ⚠️ `--parent-app` 值**固定**为 tFold Collection 的 AppId，不接受替换。
...
> tFold Collection 包含以下子应用，请挑一个具体的子流程：
>
> ① **<AppName 1>** (`<AppId 1>`) — <AppDesc 简要> · NF 版本候选: <NextflowVersion[0]>, ...
> ② **<AppName 2>** (`<AppId 2>`) — <AppDesc 简要> · NF 版本候选: ...
> ...
>
> 你想跑哪一个？（请直接告诉我编号或 AppName；常见选择为 tFold-Ab / tFold-Ag / tFold-TCR）
...
- tFold-Ag 系列含三个子流程（complex / gen MSA / inter-chain feature）含义不同——若用户笼统说"跑 tFold-Ag"，
  必须再问明确是哪一个，**禁止替选**
...
>
> - `<workflow>.antibody_fasta`：File（tFold-Ab 必填，抗体序列输入）
> - `<workflow>.antigen_fasta`：File（tFold-Ag 必填，抗原序列输入）
> - `<workflow>.precomputed_msa`：File（tFold-Ag complex 模式必填）
>
...
| `公共应用 X 是一个合集` / `AppId 不存在`  | 立即核对：`--public-app` 是否传成了合集 AppId 而非子应用 AppId？合集 AppId（`807fc9ae-...`）不可作为 run 的 `--public-app` |
...
紧凑总结：`最近 N 个 tFold 批次：✅ 已完成 X / ❌ 已失败 Y / 🔄 运行中 Z`。

> ⚠️ `omics status` 列出的是**当前项目**所有批次（不限于 tFold），SKILL 转述时
> 应**优先突出 tFold 相关批次**，但不强制过滤——这是平台原生能力。
...
### 场景 B：跑 tFold Collection 下的某子应用（标准链路）

1. `whoami` ✓ + `config show` ✓ → 复述当前配置
2. 用户："跑 tFold-Ab" 或 "做抗体结构预测"
3. **§3 展开合集**：`list public-apps --parent-app 807fc9ae-... -o json` → 把子应用清单转述给用户
4. 用户挑定子应用（例如 `tFold-Ab`、`tFold-Ag complex`、`tFold-TCR`）→ 记录 `<selectedAppId>` / `<selectedAppName>` / `<selectedNfVersion>`
...
1. 走完场景 B 第 1~5 步
2. 用户："参数我想改 antibody_fasta 文件路径"（或 antigen_fasta、precomputed_msa 等）
...
1. **拒绝**："本 SKILL 只能帮你运行 tFold Collection 下的子应用。如需运行其他应用（本地 WDL / 其他公共应用 / 项目内已有应用），
   请使用通用版的 `omics-task-skill`。"
2. 不调任何命令。

### 场景 E：用户说"直接跑 tFold 合集"不挑子应用

1. **拒绝**：合集（APP_COLLECTION）本身不可直接运行，必须挑一个具体子应用。
2. 回到 §3 展开后让用户挑。

### 场景 F：用户笼统说"跑 tFold-Ag" / "MSA 都搞了"

1. **不替选**：tFold-Ag 系列含三个子流程（complex / gen MSA / inter-chain feature），用途不同：
   - `tFold-Ag antibody-antigen complex`：根据 Pre-computed MSA 预测复合物
   - `tFold-Ag gen MSA`：使用 MMseqs2 生成 MSA（前置步骤）
   - `tFold-Ag predicting with inter-chain feature`：加入链间特征的复合物预测
2. 明确询问用户要跑哪一个；若用户想"先 gen MSA 再跑 complex"，告知 SKILL 一次只跑一个，请分两次发起。
...
tFold Collection 子应用支持 GPU 资源（T4 及以上），可通过用户的 `--input run.json` 内 nextflow config 段配置：

- `process.resourceLabels = ["gpuType":"T4", "gpuCount": "1"]`

> SKILL 不主动替用户决定 GPU 型号与数量；如用户问"用哪个"，告知候选并询问其偏好与数据规模
> （特别是 tFold-Ag complex / tFold-TCR 类大型复合物结构预测可能需要更高规格 GPU 与内存）。
