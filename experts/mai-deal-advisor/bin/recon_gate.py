...
用法: recon_gate.py <file.xlsx|docx|txt> [更多文件...]
退出码: 有勾稽问题 -> 1; 通过 -> 0
...
def find_tables(lines):
    ...
    while i < len(lines):
        ...
        if sc is not None and pc is not None and sc != pc:
            ...
            tbl = {"shares_col": sc, "pct_col": pc, "unit": unit, "hdr_line": i+1, "rows": [], "totals": []}
            i += 1
            while i < len(lines):
                ln = lines[i]
                ...
                if sval is None or pval is None: i += 1; continue
                ...
                i += 1
            if len(tbl["rows"]) >= 2: tables.append(tbl)
            continue
        i += 1
    return tables
...
def check_file(path):
    ...
    if not tables:
        print("  (未识别到含'股数列+比例列'的持股表; 若持股表无标准表头, 请整理后再校验)"); return 0
    return sum(check_table(t, k+1) for k, t in enumerate(tables))
