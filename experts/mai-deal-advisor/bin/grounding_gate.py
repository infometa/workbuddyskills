...
用法:
  grounding_gate.py <file.xlsx|file.docx|file.txt> [更多文件...]
退出码: 有应拦截项 -> 1; 全部达标 -> 0  (便于接入收尾脚本)
...
# ---------- 来源层级词表 ----------
TERMINAL  = re.compile(r'年报|年度报告|中期报告|招股书|招股章程|经审计|审计报告|财务报表|收购守则|上市规则|第\s*\d+\s*页|p\.?\s*\d+\b')
AUXILIARY = re.compile(r'broker|券商|富途|futu|hafoo|哈富|披露易|DION|DI数据库|HKEx|HKEX|wikipedia|维基|新浪|东方财富|百度|雪球|同花顺')
ASSUMPTION= re.compile(r'假设|测算|拟|协商确定|客户口径|客户最新口径|目标|情景|预计|估计|约定|建议|锁定值')
DEAL_DOC  = re.compile(r'公告|备忘录|MOU|Term\s*Sheet|协议|合同')

# ---------- 区块切分 / 类型 ----------
HEADER      = re.compile(r'^\s*[一二三四五六七八九十]+\s*、|^\s*【|方案\s*Step|Step\s*2\s*完成后|^\s*\d+\.\s')
...
DERIVED_LINE= re.compile(r'情景|Step\s*2持股|增长|倍率|持股价值|隐含股价|投入基数|平台市值|拆分|÷|募集|增发|认购|增量回报')
        ...
        for t in d.tables:
            for r in t.rows:
                ...
                if cells:
                    lines.append(" | ".join(cells))
        return lines
    raise ValueError(f"不支持的文件类型: {ext} (仅 .xlsx/.docx/.txt/.md)")
...
def segment(lines):
    ...
    for b in blocks:
        ...
        b["has_terminal"] = bool(TERMINAL.search(text))
        first = b["lines"][0][1]
        b["kind"] = "DERIVED" if (DERIVED_HDR.search(text) and not FACT_HDR.search(first)) \
                    else ("FACT" if FACT_HDR.search(text) else "DERIVED")
        for i, _ in b["lines"]: m[i] = b
    return m

def verdict(line, ctype, block):
    has_term = bool(TERMINAL.search(line)); has_aux = bool(AUXILIARY.search(line))
    has_asm  = bool(ASSUMPTION.search(line)); has_deal = bool(DEAL_DOC.search(line))
    inherit  = bool(block and block.get("has_terminal"))
    derived  = bool((block and block["kind"] == "DERIVED") or DERIVED_LINE.search(line))
    if ctype in ("SHAREHOLDING", "SHARE_CAPITAL"):
        if derived:   return "OK_DERIVED", "测算/推导输出(正确性走勾稽校验)"
    ...
    if ctype == "VALUATION":
        ...
        return "UNANCHORED", "估值/对价无来源且未标假设→待确认"
