...
def call_sse_api(question: str, session_id: str,
    ...
    return _parse_sse_stream(resp, on_event, action, cred_source, session_id)


def _parse_sse_stream(resp, on_event, action: str, cred_source: str = "env", session_id: str = "") -> dict:
    """
...
def main():
    ...
    # ---- SessionID 决策逻辑 ----
    # --session-id 为必填参数，由调用方生成 UUID v4 传入
    ...
    # 把本次使用的 session_id 回显到 stderr 首行，供调用方追问时复用
