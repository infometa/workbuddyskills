...
| `fin_data__get_latest_snapshot` | `ticker`；`market=a_stock/index` | A 股或指数最新可用快照；必须展示返回日期时间 |
...
示例：`fin_data__search_security(keyword="康哲药业", market="hk_stock", limit=5)`；解析后再调用 `fin_data__get_kline_series(ticker="0867.HK", market="hk_stock", limit=30)`。
...
| `doc_search__get_document` | 搜索返回的 `doc_id`；`doc_type`；`max_length` | 用户需要已选文档正文或详情时调用 |
...
2. 用列表/搜索工具取得少量候选，通常 `limit=5-10`。
3. 仅对用户选中或最终入选的候选调用详情、正文、图表或链接工具。
4. 需要综合判断时至少交叉核对两类独立证据，并对齐实体、日期和口径。
5. 工具返回空结果时只说明当前筛选未召回；工具报错时说明来源暂不可用，不把报错写成“没有数据”。

## 覆盖边界

- 最新快照当前以 A 股和指数为主；港股可使用证券解析和日 K，不能把快照缺口写成停牌或无行情。
