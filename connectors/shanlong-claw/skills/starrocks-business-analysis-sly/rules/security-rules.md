...
> ⚠️ **禁止**会话初始化时对 POS 视图做全表 `COUNT(*)`（40 亿行，浪费积分且易误判）。
> **正常首问直接执行业务 SQL。** 仅当首条业务 SQL 报**连接失败 / 表不存在 / 端点不可达**类错误时，再执行下方轻量探活。
...
| 条件 | 处理方式 |
|------|---------|
| 业务 SQL / 探活报连接失败、表不存在、端点错误 | 停止查询，向用户报告 MCP 连接异常（用下方模板） |
| 探活返回 0 行（权限内近 30 天无 POS） | **不**判 MCP 空库；按业务空结果（类型 B）处理，会员/供应链等其他表仍可继续查 |
| 探活返回有行 | 说明连接正常；回到业务空结果 / 类型 A 修正规则 |

**标准报错输出**（仅连接/端点类错误时原样输出，不得重试）：

```
⚠️ MCP 连接异常：无法连接数据仓库或表不存在，
疑似 MCP 端点配置错误。
...
> 💡 本规则防止 OpenClaw / WorkBuddy 环境下端点错误导致反复空查；同时避免「POS 无数据却有会员/供应链数据」被误判为 MCP 空库。

### 空结果（业务查询）

SQL 返回 0 条记录时（**排除上述 MCP 探活已确认连接异常并已停止的场景**）：
