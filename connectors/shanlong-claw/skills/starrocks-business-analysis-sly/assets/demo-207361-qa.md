# 民谣集烧烤酒馆 (group_code=207361) 问数测试集
...
SELECT
    COUNT(DISTINCT bill_id) AS 账单数,
    ROUND(SUM(person_num), 2) AS 用餐总人数,
    ROUND(SUM(person_num) / COUNT(DISTINCT bill_id), 2) AS 人均人数,
    ROUND(SUM(busi_income) / SUM(person_num), 2) AS 人均消费,
    ROUND(SUM(busi_income) / COUNT(DISTINCT bill_id), 2) AS 客单价
FROM dm.v_pos_corp_sale_analysis_with_sly
WHERE cy7_group_code = '207361'
    AND settle_biz_date >= '2026-03-01'
...
```sql
SELECT
    COUNT(DISTINCT CASE WHEN is_applet_bill = '1' THEN bill_id END) AS 小程序账单,
    COUNT(DISTINCT bill_id) AS 总账单,
...
**回答要点**：民谣集作为快速扩张品牌，同比增速是评估扩张效果的核心指标。
...
SELECT
    waiter_name AS 服务员,
    store_name AS 门店,
    COUNT(DISTINCT bill_id) AS 服务账单数,
    ROUND(SUM(busi_income), 2) AS 实收金额,
    ROUND(SUM(busi_income) / COUNT(DISTINCT bill_id), 2) AS 客单价
FROM dm.v_pos_corp_sale_analysis_with_sly
WHERE cy7_group_code = '207361'
