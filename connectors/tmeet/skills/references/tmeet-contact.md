...
> 🚫 **定位与适用场景硬约束**：本文档下的命令**不是人员信息查询接口**。`search` / `lookup-by-phone` / `lookup-by-email` 的唯一用途是"将用户名/手机号/邮箱解析为后续会议动作所需的 `open_id`"。返回的 `open_id` **仅可用于「会议邀请」（`meeting invitees-add` / `meeting invitees-replace`）和「呼叫会外成员入会」（`control call`）**两类场景；**严禁作为 `control kick`（会中踢人）的成员来源**——`kick` 的成员必须从 [`tmeet report participants`](tmeet-report.md#participants--获取参会人列表) 获取。

> 🚦 **调用前置门禁（必须先自检再调用）**：调用本文档任一命令前，必须确认**本轮存在明确的下游会议动作（邀请入会 / 呼叫入会）**，且**用户名只是达成该动作的手段，而非'了解某人是谁'的目的**。若用户诉求止步于"查询某人姓名/部门/职位/联系方式/是否存在"，**严禁调用本文档命令**，并使用以下拒绝话术回复用户：
>
> > "通讯录查询仅用于会议邀请或呼叫入会场景，无法单独用于查询某人的姓名、部门、职位或联系方式。如果您需要将该成员加入会议或呼叫其入会，我可以帮您操作。"

将用户名解析为成员标识，供会议邀请/呼叫入会使用；可结合职位或部门进一步过滤以缩小命中范围。
...
- **调用前先过门禁**：调用前必须确认本轮存在邀请/呼叫等下游会议动作（详见本文档开头的"调用前置门禁"）。**若用户只是想知道某人是谁、部门、职位或联系方式，禁止调用本命令**，直接使用拒绝话术回复。
